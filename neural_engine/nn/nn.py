from neural_engine import random, Tensor
...
class Base:
    def __repr__(self):
        layers = (
            f"({layer_name}): {layer}" for layer_name, layer in vars(self).items()
        )
...
class Layer:
    def __init__(self, in_features, no_of_neurons, bias=True):
        ...
        self._b = random.random_sample((no_of_neurons,)) if bias else None
